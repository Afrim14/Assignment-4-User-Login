...
function initializeApp() {
    ...
    if (flatpickr && document.getElementById('date-played')) {
    ...
    // Add event listeners
    if (newRoundBtn) {
        newRoundBtn.addEventListener('click', showNewScorecardModal);
    }
    
    if (scorecardForm) {
        scorecardForm.addEventListener('submit', handleFormSubmit);
    }
    ...
    if (courseFilter) courseFilter.addEventListener('change', filterScorecards);
    if (dateFilter) dateFilter.addEventListener('change', filterScorecards);
    if (searchInput) searchInput.addEventListener('input', filterScorecards);
}

// Charts
function initCharts(scorecardsData) {
    if (scorecardsData.length === 0 || !scoreTrendChart || !coursesChart) return;
...
function updateStats(scorecardsData) {
    ...
    if (totalRounds === 0) {
        ...
        if (coursesPlayedEl) coursesPlayedEl.textContent = '0';
        return;
    }
    ...
    if (avgScoreEl) avgScoreEl.textContent = formattedAvg;
    if (bestScoreEl) bestScoreEl.textContent = formattedBest;
    if (coursesPlayedEl) coursesPlayedEl.textContent = uniqueCourses.size;
}
...
async function fetchScorecards() {
    try {
        ...
        // Initialize charts
        if (Chart && scoreTrendChart && Chart.getChart(scoreTrendChart)) {
            Chart.getChart(scoreTrendChart).destroy();
        }
        if (Chart && coursesChart && Chart.getChart(coursesChart)) {
...
function filterScorecards() {
    if (!courseFilter || !dateFilter || !searchInput) return;
...
function showNewScorecardModal() {
...
function updateScorecardTotals() {
    if (!outParEl || !outScoreEl || !inParEl || !inScoreEl || !totalParEl || !totalScoreEl || !relativeToParEl) return;
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function showDashboard() {
        ...
        if (scorecardFilters) scorecardFilters.style.display = 'flex';
        if (scorecardList) scorecardList.style.display = 'block';
        
        // Update header title
        const headerTitle = document.querySelector('.header-title h1');
        if (headerTitle) headerTitle.textContent = 'Golf Scorecard Dashboard';
    }
    ...
    function showScoreHistory() {
        ...
        // Update header title
        const headerTitle = document.querySelector('.header-title h1');
        if (headerTitle) headerTitle.textContent = 'Score History';
    }
    ...
    function showStatistics() {
        ...
        if (dashboardStats) dashboardStats.style.display = 'grid';
        if (chartsContainer) chartsContainer.style.display = 'grid';
        if (scorecardFilters) scorecardFilters.style.display = 'none';
        if (scorecardList) scorecardList.style.display = 'none';
        
        // Update header title
        const headerTitle = document.querySelector('.header-title h1');
        if (headerTitle) headerTitle.textContent = 'Performance Statistics';
    }
    ...
    function showCourses() {
        ...
        if (document.querySelectorAll('.chart-card')) {
        ...
        // Update header title
        const headerTitle = document.querySelector('.header-title h1');
        if (headerTitle) headerTitle.textContent = 'Golf Courses';
    }
